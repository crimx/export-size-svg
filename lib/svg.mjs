...
/**
 * @param {import("./main").SVGOptions} options
 * @param {{ title: string, size: number }[]} sizes
 * @returns {string}
 * @see {@link https://github.com/anuraghazra/github-readme-stats}
 */
export function generateSVG(options, sizes) {
  const {
    titleColor = "#bd93f9",
    backgroundColor = "#282a36",
    cardWidth = 300,
    baselineSize = sizes.reduce((acc, { size }) => Math.max(acc, size), 0),
  } = options;
  ...
  ${
    ...
    <g transform="translate(0, 0)">
      <text x="0" y="0" class="header">${options.title}</text>
    </g>
  </g>`
      : ""
  }
...
function sizeItem(options, cardWidth, baselineSize, i, title, size) {
  const { progressColor = "#bd93f9" } = options;
  ...
  <g class="stagger" style="animation-delay: ${staggerDelay}ms">
    ...
    <svg width="${progressWidth}" x="0" y="25">
      <rect rx="5" ry="5" x="0" y="0" width="${progressWidth}" height="8" fill="#ddd"></rect>
